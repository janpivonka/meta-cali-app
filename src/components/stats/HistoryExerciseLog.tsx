...
import { Video, Activity, MessageSquare } from 'lucide-react';
...
interface HistoryExerciseLogProps {
  log: ExerciseLog;
  onMediaClick: (media: ExerciseMedia[], index: number) => void;
}

export const HistoryExerciseLog: React.FC<HistoryExerciseLogProps> = React.memo(({ log, onMediaClick }) => {
      ...
      <div>
        ...
        <div className="flex overflow-x-auto gap-4 pb-6 snap-x no-scrollbar">
          {groups.map((group, gi) => {
            ...
            const unit = isHoldExercise(metadata.exerciseId) ? "s" : "R";
            ...
            const exName = EXERCISE_LIBRARY.find(e => e.id === metadata.exerciseId)?.name || log.type;
                ...
                <div className="flex flex-col gap-y-0.5 pr-2 select-none">
                  ...
                  {(hasGroupNotes || items.some((s: any) => s.notes)) && (
                    <div className="mt-1 flex flex-col gap-1">
                      ...
                      {items.map((s: any, idx: number) => s.notes && (
                        <p key={idx} className="text-[8px] italic font-medium text-slate-500 pl-2">
                          Series #{gi + 1}.{idx + 1}: "{s.notes}"
                        </p>
                      ))}
                    </div>
                  )}
                    ...
                    {groupMedia.length > 0 && (
                      <div className="flex gap-1 overflow-x-auto no-scrollbar">
                        {groupMedia.map((m: any, midx: number) => (
                          <div
                            key={`gm-${midx}`}
                            className="w-8 h-8 rounded-lg overflow-hidden bg-black/40 border border-cyan-500/30 shrink-0 relative group/history-thumb cursor-pointer hover:border-cyan-500 transition-all shadow-lg"
                            onClick={(e) => {
                              e.stopPropagation();
                              onMediaClick(groupMedia, midx);
                            }}
                          >
                            {isMediaVideo(m) ? (
                              <div className="w-full h-full relative">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover/history-thumb:bg-black/0 transition-all">
                    ...
                    {media.length > 0 && (
                      <div className="flex gap-1 overflow-x-auto no-scrollbar">
                        {media.map((m: any, midx: number) => (
                          <div
                            key={midx}
                            className="w-6 h-6 rounded-md overflow-hidden bg-black/40 border border-white/5 shrink-0 relative group/history-thumb cursor-pointer hover:border-cyan-500/50 transition-all"
                            onClick={(e) => {
                              e.stopPropagation();
                              onMediaClick(media, midx);
                            }}
                          >
                            {isMediaVideo(m) ? (
                              <div className="w-full h-full relative">
            ...
            {log.media && log.media.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {log.media.map((m: any, midx: number) => (
                  <div 
                    key={midx} 
                    className="w-16 h-16 rounded-xl overflow-hidden border border-white/10 bg-black/40 shrink-0 cursor-pointer relative group/history-thumb hover:border-cyan-500/50 transition-all shadow-lg"
                    onClick={(e) => {
                      e.stopPropagation();
                      onMediaClick(log.media!, midx);
                    }}
                  >
                    {isMediaVideo(m) ? (
                      <div className="w-full h-full relative">
                      ...
                      <MediaRenderer url={m.url} type={m.type} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    )}
                  </div>
                ))}
              </div>
...
HistoryExerciseLog.displayName = 'HistoryExerciseLog';
