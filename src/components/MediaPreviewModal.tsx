...
export const MediaPreviewModal: React.FC<MediaPreviewModalProps> = ({ 
  ...
  const handleCaptureFrame = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!videoRef.current || !onSelectThumbnail || !media || media.length === 0) return;

    const video = videoRef.current;
    
    // Ensure video is ready
    if (video.readyState < 2) {
      console.warn('Video not ready for capture. Please try again in a moment.');
      return;
    }
    ...
    const safeIndex = currentIndex >= 0 && currentIndex < media.length ? currentIndex : 0;
    const currentMedia = media[safeIndex];
    if (currentMedia) {
      onSelectThumbnail(currentMedia, thumbnail);
    }
  };
            ...
            >
              ...
              >
                <div className="relative w-full h-full flex items-center justify-center">
                  <MediaRenderer
                    ref={videoRef}
                    key={`${safeIndex}-${currentMedia.type}-${currentMedia.id || (typeof currentMedia.url === 'string' ? currentMedia.url : safeIndex)}`}
                    url={currentMedia.url}
                    type={currentMedia.type}
                    thumbnail={currentMedia.thumbnail}
                    className="max-w-full max-h-[85vh] object-contain rounded-2xl shadow-2xl"
                    controls={isMediaVideo(currentMedia)}
                    autoPlay={isMediaVideo(currentMedia)}
                    playsInline
                    preload="metadata"
                    muted={true}
                  />

                  {isMediaVideo(currentMedia) && onSelectThumbnail && (
                    <button
                  ...
                  {media.map((_, idx) => (
                    <div
                      ...
                      className={cn(
                        ...
                        idx === safeIndex ? "bg-cyan-500 w-4" : "bg-white/20"
                      )}
                    />
                  ))}
                </div>
